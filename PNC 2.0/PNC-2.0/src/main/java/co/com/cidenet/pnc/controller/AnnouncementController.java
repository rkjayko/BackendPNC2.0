...
public class AnnouncementController {
  ...
  public ResponseEntity<Announcement> show(@PathVariable Long id) {
      ...
      try {
          ...
          response.put(Constants.ERROR, Objects.requireNonNull(e.getMessage()).concat(": ").concat(e.getMostSpecificCause().getMessage()));
          return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
      }
      if (announcement == null) {
          response.put(Constants.MESSAGE, "El ID del producto: ".concat(id.toString().concat(Constants.MSG_ERROR_NOT_EXIST)));
